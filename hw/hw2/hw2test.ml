...
let test_1 = 
  ((parse_prefix awkish_grammar accept_all ["9";"+";"3"])
   = Some ([(Expr, [N Term; N Binop; N Expr]); (Term, [N Num]); (Num, [T "9"]); (Binop, [T"+"]); (Expr, [N Term]); (Term, [N Num]); (Num, [T"3"])], []))

let test_2 = 
  ((parse_prefix awkish_grammar accept_all ["(";"(";"3";"+";"5";")";"-";"4";")"]) 
    = Some ([(Expr, [N Term]); (Term, [T "("; N Expr; T ")"]);
   (Expr, [N Term; N Binop; N Expr]); (Term, [T "("; N Expr; T ")"]);
   (Expr, [N Term; N Binop; N Expr]); (Term, [N Num]); (Num, [T "3"]);
   (Binop, [T "+"]); (Expr, [N Term]); (Term, [N Num]); (Num, [T "5"]);
